...
from typing import Dict, Sequence, Sized, Union
...
@dataclass
class DataCollatorForImage(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        # Probably later should do things such as pad sequence
        # But here for simplicity I only do the batch size 1 for now
        images, input_ids = tuple(
            [instance[key] for instance in instances] for key in ("image", "input_ids")
        )
        images = [
            to_pil_image(image).convert("RGB") if image is not None else None
            for image in images
        ]
        input_ids = [_input_ids.squeeze(0) for _input_ids in input_ids]
        input_ids = [
        ...
        return dict(images=images, input_ids=input_ids, attention_mask=attention_mask)
    ...
    def fit(self):
        ...
        if self.cfg.mm_data:
            dl = DataLoader(
                ...
                collate_fn=DataCollatorForImage(
        ...
        for batch in dl:
            ...
            for name, hiddens in hidden_dict.items():
                # When I use quantized model, the hiddens are torch.float16
                # This could cause overflow when calculating var,
                # so I convert to bfloat16
