...
from .data import MemmapDataset, chunk_and_tokenize, process_mm_data
...
class RunConfig(TrainConfig):
    ...
    data_preprocessing_num_proc: int = field(
        default_factory=lambda: cpu_count() // 4,
    )
...
def run():
    ...
    if ddp:
        ...
        if rank != 0:
            model, dataset = load_artifacts(args, rank)
        select_columns = len(dataset) - (len(dataset) % dist.get_world_size())
        dataset = dataset.select(range(select_columns))
