...
async def _simulate_and_score_sequence(
    ...
    """Score an explanation of a neuron by
    how well it predicts activations on a sentence."""
...
def aggregate_scored_sequence_simulations(
    ...
    """
    Aggregate a list of scored sequence simulations.
    The logic for doing this is non-trivial for EV
    scores, since we want to calculate the correlation
    over all activations from all sequences at
...
async def simulate_and_score(
    ...
    """
    Score an explanation of a neuron by how well
    it predicts activations on the given text
    sequences.
    """
